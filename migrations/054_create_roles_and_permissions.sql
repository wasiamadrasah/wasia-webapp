-- Migration 054: Create Roles and Permissions Tables
...
-- 6. Policies for Authenticated Admin Users
DROP POLICY IF EXISTS "Admins view and manage roles" ON public.roles;
CREATE POLICY "Admins view and manage roles" ON public.roles
  FOR ALL TO authenticated
  USING (
    EXISTS (SELECT 1 FROM public.admin_roles WHERE user_id = auth.uid()::text)
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM public.admin_roles WHERE user_id = auth.uid()::text)
  );

DROP POLICY IF EXISTS "Admins view and manage role permissions" ON public.role_permissions;
CREATE POLICY "Admins view and manage role permissions" ON public.role_permissions
  FOR ALL TO authenticated
  USING (
    EXISTS (SELECT 1 FROM public.admin_roles WHERE user_id = auth.uid()::text)
  )
...
-- 8. Seed Default Permissions for Super Admin & Admin
DO $$
DECLARE
  superadmin_id UUID;
  admin_id UUID;
